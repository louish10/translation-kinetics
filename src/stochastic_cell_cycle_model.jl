...
function main(args)
    ...
    global N = parse(Float64, args[6])
    global k = N/T

    global path = mkpath("data/stochastic-cell-cycle-model/alph-$alpha-bet-$beta-gam-$gamma-rho-$rho-T-$T-N-$N")
        ...
        elseif R < sum(instantaneous_propensities[1:4])/ a
            ...
            if j == N
                if steady_state
                    ...
                    append!(protein_numbers, species[2])
                    append!(p_time_averages, 1/time_since_division*sum(protein_numbers .* time_intervals))
                    append!(p_squared_time_averages, 1/time_since_division*sum(protein_numbers .^2 .*time_intervals))

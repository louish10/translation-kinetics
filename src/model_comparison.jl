...
function main()
    ...
    for i in [0.1, 0.5, 1.0, 10.0, 50.0, 100.0, 150.0]
        ...
        heatmap(rhoT, betaT, protein_var_model_1, c = :thermal, title = "Protein Number for Canonical Two Stage Model")
        xlabel!(L"\rho T")
        ylabel!(L"\beta T")
        savefig(string(path, "/protein_variance_model_1.svg"))

        heatmap(rhoT, betaT, protein_var_model_2, c = :thermal, title = "Protein Number for Model Including Ribosome Kinetics")
        xlabel!(L"\rho T")
        ylabel!(L"\beta T")
